...
m=Dialog.getNumber();
n=Dialog.getNumber(); //n=time points; m=positions
...
//Save ROI
roiManager("save", dir + expID + "_RoiSet.zip");
...
saveAs("Results", dir + expID + "_ROIData.csv");
...
//Gets Image Barcodes
	OpenFile("405Ex.tif");			//Opens files & ROIs and gets barcode strips
	OpenFile("633Ex_Linear unmixing.tif");
	setBatchMode(true);
  ...
  if (list.length==0)
	...
	run("Make Montage...", "columns=m rows=1 scale=1");
	close(title_4);
...
OpenFile("633Ex.tif");			//Opens files & ROIs and gets spectral data
roiManager("Deselect");	
run("Make Montage...", "columns=m rows=1 scale=1");
